...
describe('Ticket e2e test', () => {
  ...
  const ticketSample = { title: 'Деревянный интеграционный', description: 'generate project users' };
  ...
  describe('new Ticket page', () => {
    ...
    it('should create an instance of Ticket', () => {
      cy.get(`[data-cy="title"]`).type('Stand-alone Неодимовый Bedfordshire').should('have.value', 'Stand-alone Неодимовый Bedfordshire');

      cy.get(`[data-cy="description"]`).type('техник').should('have.value', 'техник');

      cy.get(`[data-cy="dueDate"]`).type('2022-08-16').blur().should('have.value', '2022-08-16');

      cy.get(`[data-cy="done"]`).should('not.be.checked');
      cy.get(`[data-cy="done"]`).click().should('be.checked');

      cy.get(`[data-cy="severity"]`).select('MINOR');
